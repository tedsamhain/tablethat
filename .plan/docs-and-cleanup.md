...
Purpose: Several docs are stale after recent refactors. Dead code accumulates. Cross-platform portability has gaps.
...
- Remove `open_preview()` from tui\_kanban.rs (dead code, replaced by OpenGloss)
- Deduplicate `status_color`/`priority_color` — currently in both tasks.rs and tui\_kanban.rs, move to lib.rs
